/**
 * Sakthi Textile shop hostname on Cloudflare → Vercel origin.
 *
 ...
 *
 * X-Sakthi-Cache: HIT | MISS | BYPASS
 *
 * Canonical host is www (matches Vercel apex → www redirect).
 */
const ORIGIN = "https://st-sarees.vercel.app";
const APEX = "sakthitextile.com";
const WWW = "www.sakthitextile.com";
...
function rewriteLocation(value: string, incomingHost: string): string {
  try {
    ...
    if (
      url.hostname === "st-sarees.vercel.app" ||
      url.hostname.endsWith(".vercel.app")
    ) {
      url.protocol = "https:";
      url.hostname = incomingHost === APEX ? WWW : incomingHost;
    }
    return url.toString();
  } catch {
    return value;
...
function isPublicStorefrontApi(pathname: string): boolean {
  ...
  if (path === "/api/geo/pincode") return true;
  return false;
}
...
/** Stable cache key: www host; size-config productIds sorted. */
...
function parseSMaxAge(cacheControl: string | null, fallback: number): number {
...
function withCacheHeader(response: Response, value: string): Response {
  ...
  headers.set("X-Sakthi-Cache", value);
...
async function handleCachedGet(
  ...
  storeHeaders.set("X-Sakthi-Cache", "MISS");
