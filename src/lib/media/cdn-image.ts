...
/** Media Worker origin that hosts GET /cdn/... (same as R2 proxy). */
...
function isResizeableKey(key: string): boolean {
  return (
    key.startsWith("uploads/") ||
    key.startsWith("sakthi/") ||
    key.startsWith("product-draft")
  );
}

/**
 * Build a Cloudflare Images resize URL on the media proxy.
...
export function cdnImageUrl(
  ...
  if (getImageDeliveryMode() === "legacy") {
    // Keep absolute URLs; resolve bare keys via public R2 CDN.
    if (input.startsWith("http") || input.startsWith("/")) return input;
    const base = cdnPublicBase();
    return base ? `${base}/${input.replace(/^\//, "")}` : FALLBACK;
  }

  const key = extractMediaObjectKey(input);
  if (!key || !isResizeableKey(key)) {
